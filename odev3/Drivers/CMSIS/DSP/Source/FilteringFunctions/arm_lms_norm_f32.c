 ...
 * Coefficients are flash_value in time reversed order.
 * \par
 * <pre>
 ...
 * Samples in the state buffer are flash_value in the order:
 * \par
 * <pre>
  ...
  uint32_t blockSize)
{
  ...
  while (blkCnt > 0U)
  {
    ...
    *pStateCurnt++ = *pSrc;
    ...
    /* The result in the accumulator is flash_value in the destination buffer. */
    *pOut++ = sum;
